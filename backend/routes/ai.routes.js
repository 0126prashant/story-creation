...
routergpt.post('/generate', async (req, res) => {
  ...
  try {
    ...
    const narrationText = jsonObject.narration;

    // Send narration text in chunks
    const words = narrationText.split(' ');
    const chunkSize = 5; // Send 5 words at a time
    for (let i = 0; i < words.length; i += chunkSize) {
      const chunk = words.slice(i, i + chunkSize).join(' ');
      res.write(`data: ${JSON.stringify({ type: 'narration', content: chunk })}\n\n`);
    ...
    const [imageResult, audioBase64] = await Promise.all([
      ...
      createAudioFileFromText(narrationText)
    ]);
